...
def log_reg_mod_tuning(X_train, X_test, y_train, y_test, accuracyDict):
    print("\nTuning the Logistic Regression Model with RandomizedSearchCV\n")
    parameter = {"C": sp_randint(1,100),
                  "solver": ["newton-cg", "lbfgs", "sag"],
                  "multi_class": ["ovr", "multinomial"],
                  "max_iter": sp_randint(100,500)}
    random_search = RandomizedSearchCV(LogisticRegression(), parameter, cv=5, scoring='accuracy')
...
def tuneKNN(X_train, X_test, y_train, y_test, accuracyDict):
    ...
    parameter = {"n_neighbors": sp_randint(1,100),
                  "weights": ["uniform", "distance"],
                  "algorithm": ["auto", "ball_tree", "kd_tree", "brute"],
                  "leaf_size": sp_randint(10,100)}
    random_search = RandomizedSearchCV(KNeighborsClassifier(), parameter, cv=5)
    random_search.fit(X_train,y_train)
    print("Best parameters: ", random_search.best_params_)
    print("Best cross-validation score: ", random_search.best_score_*100, "%")
    print("Best estimator: ", random_search.best_estimator_)
...
def tuneDT(X_train, X_test, y_train, y_test, accuracyDict):
    ...
    parameter = {"criterion": ["gini", "entropy"],
                  ...
                  "random_state": [0]}
    random_search = RandomizedSearchCV(DecisionTreeClassifier(), parameter, cv=5)
    random_search.fit(X_train,y_train)
    print("Best parameters: ", random_search.best_params_)
    print("Best cross-validation score: ", random_search.best_score_*100, "%")
    print("Best estimator: ", random_search.best_estimator_)
...
def tuneRF(X_train, X_test, y_train, y_test, accuracyDict):
    ...
    parameter = {"n_estimators": sp_randint(10,100),
                  "max_depth": sp_randint(1,100),
                  "min_samples_split": sp_randint(2,10),
                  "criterion": ["gini", "entropy"],
                  "random_state": [0]}
    random_search = RandomizedSearchCV(RandomForestClassifier(), parameter, cv=5)
    random_search.fit(X_train,y_train)
    print("Best parameters: ", random_search.best_params_)
    print("Best cross-validation score: ", random_search.best_score_*100, "%")
    print("Best estimator: ", random_search.best_estimator_)
...
def tuneBoosting(X_train, X_test, y_train, y_test, accuracyDict):
    ...
    parameter = {"n_estimators": sp_randint(10,100),
                  ...
                  "random_state": [0]}
    random_search = RandomizedSearchCV(AdaBoostClassifier(), parameter, cv=5)
    random_search.fit(X_train,y_train)
    print("Best parameters: ", random_search.best_params_)
    print("Best cross-validation score: ", random_search.best_score_*100, "%")
    print("Best estimator: ", random_search.best_estimator_)
...
def tuneBagging(X_train, X_test, y_train, y_test, accuracyDict):
    ...
    parameter = {"n_estimators": sp_randint(10,100),
                  ...
                  "random_state": [0]}
    random_search = RandomizedSearchCV(BaggingClassifier(), parameter, cv=5)
    random_search.fit(X_train,y_train)
    print("Best parameters: ", random_search.best_params_)
    print("Best cross-validation score: ", random_search.best_score_*100, "%")
    print("Best estimator: ", random_search.best_estimator_)
...
#                   "bootstrap_features": [True,False],
#                   "random_state": [0]}
#     random_search = RandomizedSearchCV(StackingClassifier(), parameter, cv=5)
#     random_search.fit(X_train,y_train)
#     print("Best parameters: ", random_search.best_params_)
#     print("Best cross-validation score: ", random_search.best_score_*100, "%")
#     print("Best estimator: ", random_search.best_estimator_)
